...
const CONFIG = {
  ...
  // Paste your Make.com "Custom webhook" URL here to get an email the
  // moment someone RSVPs. Leave blank to skip email notifications.
  MAKE_WEBHOOK_URL: 'https://hook.eu1.make.com/uq1lbhvred44flss6iw25alsivojdmsq',
};

function notifyWebhook(eventType, payload) {
  if (!CONFIG.MAKE_WEBHOOK_URL) return;
  // Fire-and-forget: a slow/failed notification should never block or
  // break the guest's RSVP confirmation.
  fetch(CONFIG.MAKE_WEBHOOK_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ event: eventType, record: payload }),
    ...
    try {
      ...
      if (insertError) {
        if (insertError.code === '23505') {
          // already subscribed — nothing more to do, treat as success
        } else {
          throw insertError;
        }
      }
...
(function initRsvpForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      // Try inserting first. If this email has already RSVP'd, Postgres
      // raises a unique-violation (23505) — in that case we update the
      // existing row instead. This avoids Supabase's combined INSERT+UPDATE
      // upsert path, which some projects' RLS configurations reject with a
      // 401/42501 even when both individual policies are correct.
      const { error: insertError } = await supabaseClient.from('rsvp').insert(data);
...
(function initGuestbook() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (error) throw error;
      notifyWebhook('guestbook', { name, message });
